...
import           Data.Char                  (isAlpha, isDigit, isLower, isPrint,
                                             isUpper, toUpper)
import           Data.List                  (insertBy, intercalate, partition)
...
header filename data_dir =
  ...
  , "-module(" ++ modulename ++ ").\n"
  , ""
  , if debugErlang then "" else "-compile(inline)."
  , if debugErlang then "" else "-compile({inline_size,1100})."
  , "-compile(export_all)."
  , ""
...
addRecord name arity = do
  ...
  let records'   = insertBy (comparing fst) (name,arity) records
...
getNextCheckedFnName fn arity = do
  ...
  case Map.lookup (fn,arity) checked of
    Nothing -> do
      modify (\ecg -> ecg { checked_fns = Map.insert (fn,arity) 1 checked })
      return . strAtom $ "checked_" ++ fn ++ "_" ++ show 0
    Just x -> do
      modify (\ecg -> ecg { checked_fns = Map.update (Just . (+1)) (fn,arity) checked })
